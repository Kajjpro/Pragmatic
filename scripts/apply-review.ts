...
import { textToOptions, validatePrecomputed, type Precomputed, type PrecomputedCard, type PrecomputedGroup } from "../lib/ai/precomputed";
...
for (const line of review.split("\n")) {
  ...
  if (heading) {
    currentCard = heading[1] === "##" ? data.cards.find((c) => c.key === heading[2]) : undefined;
  ...
  const owner = currentCard?.key || currentGroup?.key;
  if (!owner) {
    continue;
  }
...
// ── Устгах картуудыг хасаж, дарааллыг 1, 2, 3... болгож цэгцэлнэ ──
data.cards = data.cards
  .filter((c) => !deleteKeys.includes(c.key))
...
console.log(`→ ${join(DIR, "precomputed.json")} хадгалагдлаа (${data.cards.length} карт)`);
